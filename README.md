...
- 🎯 **精准监控**: 支持指定特定模型进行监控
- 📈 **初始报告**: 启动时自动发送当前持仓详情（包括方向、杠杆、价格、浮盈）
- 📝 **详细日志**: 完整的操作日志记录
- ⚙️ **灵活配置**: 通过环境变量进行配置管理
...
3. **初始报告**: 首次运行时，发送当前所有持仓的详细报告（包括方向、数量、杠杆、价格、浮盈）
4. **变化检测**: 与上次数据 `last.json` 进行比较
5. **交易分析**: 识别以下交易行为：
   - 新开仓（买入/卖出）
   - 平仓
   - 加仓/减仓
   - 杠杆调整
   - 模型新增/删除
6. **通知发送**: 如有变化，发送 Telegram 通知
7. **数据更新**: 将 `current.json` 重命名为 `last.json`
